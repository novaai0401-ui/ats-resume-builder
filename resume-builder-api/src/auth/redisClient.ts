...
export class RedisClientService implements RedisLikeClient {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    if (url && token && isValidRedisUrl(url) && !isPlaceholderValue(token)) {
...
/** Reject placeholder URLs from .env.example that would cause DNS failures. */
...
/** Detect common placeholder/example values copied from .env.example. */
function isPlaceholderValue(value: string): boolean {
  return /YOUR[_-]|example\.com|placeholder|change[_-]me|changeme/i.test(value);
}
