...
// Shared with the anonymous POST /public/parse-upload route (R-102) so the
// two upload paths can never drift on format or magic-byte validation.
import {
  MAX_UPLOAD_BYTES,
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      ...
      fileFilter: (req: Request & { fileValidationError?: string }, file: UploadedResumeFile, cb: (error: Error | null, acceptFile: boolean) => void) => {
        ...
        const isSupported = SUPPORTED_UPLOAD_MIME_TYPES.has(mime) || SUPPORTED_UPLOAD_EXTENSIONS.has(ext);
        if (isSupported) {
          cb(null, true);
          return;
        }
