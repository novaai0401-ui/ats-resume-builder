...
export function applyParsedResume(
  ...
  // On re-upload, fully replace the resume with newly parsed data instead of
  // merging into the previous one.  This ensures a second upload clears the
  // first resume's details and shows only the new resume's content.
  const nextResume = parsedSession.resume;
  const nextImportNotes = parsedSession.importNotes || '';
