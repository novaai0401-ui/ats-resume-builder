...
export default function CreativePortfolio({ resumeData }: TemplateProps) {
  ...
  return (
    <article className="ats-template ats-template--creative">
      ...
      <section className="ats-section">
        ...
        <p>{summary || 'Share your creative focus, style, and mediums.'}</p>
      </section>

      {projects.length ? (
        <section className="ats-section">
          <h2>FEATURED WORK & PORTFOLIO</h2>
          {projects.map((item, idx) => (
            <div className="ats-item" key={`creative-proj-${idx}`}>
              ...
              <ul>
                {cleanList(item.highlights).map((line, lineIdx) => (
                  <li key={`creative-proj-line-${idx}-${lineIdx}`}>{line}</li>
                ))}
              </ul>
            </div>
          ))}
      ...
      <section className="ats-section">
        ...
        {experience.length ? experience.map((item, idx) => (
          <div className="ats-item" key={`creative-exp-${idx}`}>
            <h3>{item.role || 'Role'}{item.company ? `, ${item.company}` : ''}</h3>
            {displayDateRange(item.startDate, item.endDate) ? <p className="ats-item__meta">{displayDateRange(item.startDate, item.endDate)}</p> : null}
            <ul>
              {cleanList(item.highlights).map((line, lineIdx) => (
                <li key={`creative-exp-line-${idx}-${lineIdx}`}>{line}</li>
              ))}
            </ul>
          </div>
      ...
      <section className="ats-section">
        ...
        {education.length ? education.map((item, idx) => (
          <div className="ats-item" key={`creative-edu-${idx}`}>
      ...
      {certifications.length ? (
        <section className="ats-section">
          ...
          {certifications.map((item, idx) => (
            <div className="ats-item" key={`creative-cert-${idx}`}>
