...
export type VerificationIssueKind =
  ...
  | 'orphan-date-range'
  | 'phantom-experience'
  | 'experience-without-role'
  | 'experience-without-company'
...
const DATE_RANGE_RE = new RegExp(
  `\\(?\\s*(${DATE_TOKEN})\\s*(?:-|to|–|—|â€”|â€“)\\s*(${PRESENT_TOKEN}|${DATE_TOKEN})\\s*\\)?`,
  'gi',
);
...
function containsLooseSubstring(haystack: string, needle: string): boolean {
...
function extractRawDateRanges(rawText: string): string[] {
...
function rangeIsRepresented(range: string, extracted: VerifiableResume): boolean {
  const rangeNorm = normalizeForCompare(range);
  if (!rangeNorm) return true;
  ...
  for (const candidate of candidates) {
    ...
    if (matchCount === tokens.length) return true;
  }
  ...
  if (/present|current|now|till\s*date/i.test(range)) {
    if (extracted.experience.some((e) => /present/i.test(e.endDate || ''))) {
      ...
      return extracted.experience.some((e) =>
        normalizeForCompare(`${e.startDate}`).includes(startNorm),
      );
    }
  }
  return false;
...
function findCandidateName(rawText: string): string {
  ...
  for (const line of lines) {
    ...
    if (/^[\p{Lu}][\p{Lu}'\-]+(\s+[\p{Lu}][\p{Lu}'\-]+){1,4}$/u.test(line) && line.length <= 56) return line;
  }
  return '';
}
 ...
 */
export function verifyExtraction(rawText: string, extracted: VerifiableResume): VerificationReport {
  ...
  if (!extracted.contact?.email) {
    ...
    if (emailMatch) issues.push({ kind: 'missing-email', detail: emailMatch[0] });
  }
  ...
  for (const exp of extracted.experience) {
    ...
    if (!company && role) issues.push({ kind: 'experience-without-company', detail: role });
    if (!role && company) issues.push({ kind: 'experience-without-role', detail: company });
  ...
  for (const range of dateRanges) {
    if (!rangeIsRepresented(range, extracted)) orphans.push(range);
  }
  ...
  // -----------------------------------------------------------------------
  const WEIGHTS: Record<VerificationIssueKind, number> = {
    'missing-name': 0.15,
    'missing-email': 0.10,
    'missing-phone': 0.08,
 ...
 */
export function pickBetterExtraction<T extends VerifiableResume>(
  primary: T,
  alternative: T,
  primaryReport: VerificationReport,
  alternativeReport: VerificationReport,
): { winner: T; report: VerificationReport; usedAlternative: boolean } {
