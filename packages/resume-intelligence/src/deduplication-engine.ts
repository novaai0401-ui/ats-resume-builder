 ...
 */
export function deduplicateExact(items: string[]): string[] {
 ...
 */
export function deduplicateFuzzy(items: string[], threshold?: number): string[] {
  const config = getExtractionConfig();
  const similarityThreshold = threshold ?? config.fuzzySimilarityThreshold;
  if (!config.fuzzyDedup) return items;
 ...
 */
export function deduplicateSubset(items: string[], minLength: number = 20): string[] {
  const config = getExtractionConfig();
  if (!config.subsetDedup) return items;
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!isSubset) result.push(items[i]);
  }
 ...
 */
export function deduplicateSkillsSemantic(skills: string[]): string[] {
 ...
 */
export function deduplicateCrossSection(
  primary: string[],
  secondary: string[],
  threshold: number = 0.9,
): string[] {
  const config = getExtractionConfig();
  if (!config.crossSectionDedup) return secondary;
  ...
  for (const item of secondary) {
    ...
    // Exact match
    if (primaryNormalized.has(norm)) continue;
 ...
 */
function normalizeCompanyForDedup(company: string): string {
  return company
    .toLowerCase()
 ...
 */
export function deduplicateExperience(entries: ExperienceItem[]): ExperienceItem[] {
  const config = getExtractionConfig();
  if (!config.enhancedDedup) return entries;
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    for (let j = i + 1; j < entries.length; j++) {
      ...
      const companyMatch =
        normalizeCompanyForDedup(current.company) === normalizeCompanyForDedup(entries[j].company) ||
      ...
      const roleSimilarity = stringSimilarity(
        ...
        entries[j].role.toLowerCase(),
      );
      if (roleSimilarity < 0.9) continue;
    ...
    current.highlights = deduplicateSubset(
      deduplicateFuzzy(deduplicateExact(current.highlights), 0.88),
      30,
    );
...
export interface DeduplicationInput {
  ...
  highlights?: string[]; // flattened experience highlights for cross-section
}
 ...
 */
export function runDeduplicationPipeline(input: DeduplicationInput): DeduplicationResult {
  const config = getExtractionConfig();
