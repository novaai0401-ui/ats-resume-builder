...
const EMAIL_OBFUSCATIONS: Array<[RegExp, string]> = [
  [/\s*\(\s*at\s*\)\s*/gi, '@'],
  [/\s*\[\s*at\s*\]\s*/gi, '@'],
  [/\s+at\s+/gi, '@'],
  [/\s*\(\s*dot\s*\)\s*/gi, '.'],
...
export function extractLinks(text: string): string[] {
  ...
  const bareHostRe = new RegExp(
    '\\b((?:' + KNOWN_SOCIAL_HOSTS.map((h) => h.replace(/\./g, '\\.')).join('|') + ')\\/[A-Za-z0-9._\\-/]+)',
    'gi',
  );
...
export function classifyLink(url: string): 'linkedin' | 'github' | 'portfolio' | 'social' | 'other' {
  const lower = String(url || '').toLowerCase();
  if (lower.includes('linkedin.com')) return 'linkedin';
  if (lower.includes('github.com') || lower.includes('gitlab.com') || lower.includes('bitbucket.org')) return 'github';
  if (KNOWN_SOCIAL_HOSTS.some((h) => lower.includes(h))) return 'social';
  if (/portfolio|website|home|me\b|\.dev|\.io|\.me/.test(lower)) return 'portfolio';
  return 'other';
}
...
export function normalizeDateFlexible(token: string): string {
  if (!token) return '';
  const cleaned = token.trim();
  if (/^(present|current|now|till\s*date|ongoing)$/i.test(cleaned)) return 'Present';
...
export const ADDITIONAL_TECH_SKILLS: string[] = [
  // AI / ML / LLM
...
export function extractAdditionalTechSkills(text: string, knownSet?: Set<string>): string[] {
  if (!text) return [];
  const found: string[] = [];
  const seen = new Set<string>(knownSet ? Array.from(knownSet).map((s) => s.toLowerCase()) : []);
  for (const skill of ADDITIONAL_TECH_SKILLS) {
...
export function hardenString(value: string): string {
...
export function isSafeUrl(url: string): boolean {
  ...
  if (JS_PROTOCOL_RE.test(url)) return false;
  if (DATA_URL_HTML_RE.test(url)) return false;
...
export function extractSpokenLanguages(lines: string[]): Array<{ name: string; proficiency?: string }> {
  ...
  for (const rawLine of lines || []) {
    ...
    for (const token of tokens) {
      ...
      if (!LANGUAGE_NAMES.has(lang)) continue;
