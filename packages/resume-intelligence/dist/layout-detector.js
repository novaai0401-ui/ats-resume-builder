...
exports.detectLayout = detectLayout;
exports.deinterleaveColumns = deinterleaveColumns;
/**
 ...
 */
function detectLayout(text, positionHints) {
 ...
 */
function detectLayoutFromText(text) {
    ...
    for (const line of lines) {
        ...
        if (/\t/.test(trimmed))
            tabSeparatedLines++;
        if (/\s{4,}/.test(trimmed) && len > 20)
            multiSpaceSeparatedLines++;
        if (len < 30)
            shortLines++;
        else if (len > 60)
            longLines++;
 ...
 */
function detectLayoutFromPositions(hints) {
 ...
 */
function deinterleaveColumns(text, layout) {
