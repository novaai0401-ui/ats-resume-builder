    ...
    for (const rawLine of source) {
        ...
        if (roleLabelMatch) {
            if (current && !current.role) {
                ...
                if (roleText && looksLikeRole(roleText))
        ...
        if (companyBareDate) {
            ...
            if (current && !current.company)
                current.company = companyBareDate.company;
        ...
        if (companyLocDate) {
            ...
            if (current && !current.company)
                current.company = companyLocDate.company;
...
function deduplicateEducation(items) {
    const seen = new Map();
...
function extractInlineLanguages(bullets) {
    const found = new Set();
...
function extractInlineCertifications(bullets) {
    const found = [];
    const seen = new Set();
    for (const raw of bullets) {
...
function extractInlineAchievements(bullets) {
    const found = [];
    const seen = new Set();
    for (const raw of bullets) {
        const line = String(raw || '').replace(/^[-*•·]\s*/, '').trim();
        if (!line || line.length < 20 || line.length > 300)
            continue;
        if (!ACHIEVEMENT_SIGNAL_RE.test(line))
            continue;
...
function mapCertifications(sections) {
    ...
    for (const rawLine of lines) {
        ...
        if (/@/.test(line) || /\b\d{7,}\b/.test(line))
            continue; // contact (email / phone)
        if (/^(linkedin|github|portfolio|website|e-?mail|phone|mobile|contact|address)\b/i.test(line))
            continue; // contact labels
        if (isStandaloneDateLine(line))
            continue; // a bare date range is not a certificate
        if (/^[a-z]/.test(line))
            continue; // lowercase start → wrapped prose fragment, not a cert name
        ...
        if (/\b(b\.?e\.?|b\.?tech|b\.?sc|b\.?a\.?|b\.?com|bba|bca|bachelor|m\.?e\.?|m\.?tech|m\.?sc|m\.?a\.?|m\.?com|mba|mca|master|ph\.?d|doctorate)\b/i.test(line)
            && !/\b(certified|certificate|certification|course|training)\b/i.test(line))
            continue;
 ...
 * into experience.
 */
function recoverClusteredExperienceBlocks(lines) {
    const CERT_NOUN_RE = /\b(certificate|certification|certified|award|license|licensed|diploma|accreditation|nanodegree)\b/i;
    const isHeaderLine = (raw) => {
        const stripped = cleanLooseText(stripDates(raw));
        if (!stripped || stripped.length < 4)
            return false;
        if (CERT_NOUN_RE.test(stripped))
            return false;
        if (looksLikeEducationRoleLine(stripped) || looksLikeEducationDegreeLine(stripped))
            return false;
        if (parseRoleCompanyPair(stripped))
            return true;
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (dateIdx < 0) {
            for (let j = i + 1; j <= Math.min(i + 2, lines.length - 1); j += 1) {
                if (isStandaloneDateLine(cleanLooseText(lines[j]))) {
                    dateIdx = j;
                    break;
                }
            }
...
function buildExperienceSource(parsed) {
    ...
    for (let i = 0; i < otherLines.length; i += 1) {
        if (!isRoleishLine(otherLines[i]))
            continue;
        if (isDateLine(otherLines[i])) {
            roleAdjacentToDate = true;
            break;
        }
        for (let j = i + 1; j <= Math.min(i + 3, otherLines.length - 1); j += 1) {
            if (isDateLine(otherLines[j])) {
                roleAdjacentToDate = true;
                break;
            }
        }
        if (roleAdjacentToDate)
            break;
    }
 ...
 */
function parseCompanyLocationDateLine(line) {
    ...
    const match = line.match(/^(.+?)\s*(?:\([^)]*?\)\s*)*\(([^)]*(?:\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{4}|\d{1,2}[/-]\d{4}|\b(?:19|20)\d{2})[^)]*)\)\s*$/i);
    if (!match)
        return null;
    ...
    if (looksLikeRole(company) && !looksLikeCompany(company))
        return null;
 ...
 */
function parseCompanyDateLine(line) {
    if (!isDateLine(line))
        return null;
    const dates = extractDates(line);
    ...
    if (!looksLikeCompany(prefix)) {
        ...
        if (tokens.length !== 1)
            return null;
        if (!/^[A-Z][A-Za-z0-9&'.-]+$/.test(tokens[0]) && !/^[A-Z]{2,6}$/.test(tokens[0]))
            return null;
    }
...
function looksLikeEducationInstitutionLine(line) {
    ...
    if (looksLikeRole(cleaned) && !/(university|college|school|institute|academy)\b/i.test(cleaned))
        return false;
    if (looksLikeCompany(cleaned) && !/(university|college|school|institute|academy)\b/i.test(cleaned))
        return false;
...
function looksLikeCompany(line) {
    ...
    if (looksLikePureLocation(cleaned))
        return false;
 ...
 */
function looksLikePureLocation(line) {
    const cleaned = cleanLooseText(line);
    if (!cleaned)
        return false;
    ...
    if (/(inc|llc|ltd|corp|company|technologies|systems|labs|solutions|group|studio|partners|bank|consulting|digital|enterprises|pvt|limited|infotech)\b/i.test(cleaned))
        return false;
    ...
    const everyPartLooksGeographic = parts.every((p) => {
        if (looksLikeLocationFragment(p))
            return true;
            ...
            for (const [existingKey, existing] of map.entries()) {
                if (existingKey.startsWith(fuzzyKey + '|')) {
                    ...
                    if (datesOverlapOrAdjacent(existing.startDate, existing.endDate, startDate, endDate)) {
                        ...
                        existing.highlights = uniqueLines([...existing.highlights, ...item.highlights.map((line) => cleanLooseText(line)).filter(Boolean)]);
                        merged = true;
                        break;
                    }
                }
...
function datesOverlapOrAdjacent(start1, end1, start2, end2) {
    ...
    if (!s1 && !e1 && !s2 && !e2)
        return true; // Both undated = likely same
    if ((!s1 && !e1) || (!s2 && !e2))
        return false; // One undated, one dated
